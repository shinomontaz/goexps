...
func GetRoute(points []Point) (float64, float64, Geojson, error) {
...
func GetMatrices(pts []Point) ([][]float64, [][]float64) {
	qsParts := make([]string, 0)
	for _, p := range pts {
		qsParts = append(qsParts, fmt.Sprintf("%f,%f", p.Lng, p.Lat))
	}
...
func GetMatrices2(pts []Point, src, dst []int) ([][]float64, [][]float64) {
	qsParts := make([]string, 0)
	for _, p := range pts {
		qsParts = append(qsParts, fmt.Sprintf("%f,%f", p.Lng, p.Lat))
	}
