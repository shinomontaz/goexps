...
func sa(initSolution [][]int, pts []Point, dm, tm [][]float64, cooling float64) [][]int {
	T := 1.0
	Tmin := 0.001
	...
	oldFitness = fitness(initSolution, pts, dm)
	...
	for T > Tmin {
		...
		for i := 0; i < 100; i++ {
			...
			newFitness = fitness(newSolution, pts, dm)
			if !isFeasible(newSolution, pts, dm, tm) {
				continue
			}
			...
			if newFitness < oldFitness {
				...
				copySlice(currSolution, newSolution)
				oldFitness = newFitness
				fmt.Println("accepted: ", oldFitness, "routes", len(currSolution))
			}
		}
