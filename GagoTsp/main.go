...
func (p Path) Mutate(rng *rand.Rand) {
	...
	randIndex1, randIndex2 := rand.Intn(len(p)-1), rand.Intn(len(p)-1)
	p[randIndex1], p[randIndex2] = p[randIndex2], p[randIndex1]
...
func main() {
	...
	fmt.Println("Path: ", ga.HallOfFame[0].Genome.(Path))
